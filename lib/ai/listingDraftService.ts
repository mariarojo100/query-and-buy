/**
 * lib/ai/listingDraftService — photo → listing-draft generation shared by the
 * web action (app/sell/aiActions.ts) and POST /api/v1/ai/listing-draft.
 * Rate limiting (12/min/user), the 70% confidence gate, the transparency
 * panel, pricing tiers, and low-confidence moderation logging live here.
 */
...
export async function generateListingDraftAs(
  viewer: Viewer,
  images: AiImageInput[],
  ...
  if (!enforceRateLimit('ai.draft', viewer.id, 12, 60_000).allowed) {
    return {
      ok: false,
      error: 'You are generating listings too quickly. Please wait a moment and try again.',
    }
  }
