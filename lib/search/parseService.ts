/**
 * lib/search/parseService — conversational search parsing shared by the web
 * action (app/search/actions.ts) and POST /api/v1/search/parse.
 * Never throws: AI failure/rate-limit falls back to the deterministic
 * heuristic parse of the raw text.
 */
import { getActiveCategories } from '@/lib/listings/queries'
import { getProvider } from '@/lib/ai/provider'
import { track } from '@/lib/analytics'
import { logger } from '@/lib/logger'
import { logSearch } from '@/lib/search/intelligence'
...
/**
 * Parse a plain-English query into validated search filters. Viewer may be
 * null (anon search is allowed); signed-in users get the per-user AI cap,
 * anon traffic relies on the heuristic fallback + provider quota.
 */
export async function parseConversationalSearchAs(
  viewer: Viewer | null,
  text: string,
  ...
  await logSearch(raw, viewer?.id ?? null)
