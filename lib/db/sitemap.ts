...
export type SitemapData = {
  ...
  listings: { publicId: string; title: string; lastModified: Date }[]
...
export async function sitemapData(): Promise<SitemapData> {
  const [cats, listings, profiles] = await Promise.all([
    ...
    db.listing.findMany({
      ...
      take: 5000,
      select: { publicId: true, titleEn: true, publishedAt: true, updatedAt: true },
    }),
    db.profile.findMany({ where: { username: { not: null } }, take: 5000, select: { username: true, updatedAt: true } }),
  ])
  return {
    categorySlugs: cats.map((c) => c.slug),
    listings: listings.map((l) => ({ publicId: l.publicId, title: l.titleEn, lastModified: l.updatedAt ?? l.publishedAt ?? new Date() })),
    profiles: profiles
